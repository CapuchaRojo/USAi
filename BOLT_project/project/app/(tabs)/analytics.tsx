...
export default function AnalyticsScreen() {
  ...
  const [swarms, setSwarms] = useState<SwarmDeployment[]>([]);
  const [refreshing, setRefreshing] = useState(false);

  const loadAllData = async () => {
    try {
      const [agentsData, missionsData, pipelinesData, swarmsData] = await Promise.all([
        api.agentService.getAll(),
        api.missionService.getAll(),
        api.ecrrService.getAll(),
        api.swarmService.getAll(),
      ]);

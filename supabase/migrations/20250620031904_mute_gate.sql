/*
  # Add Agent Skill Acquisition System

  1. New Functions
    - `update_agent_skill` - Updates individual agent skills and logs changes
    - Enhanced logging for skill acquisition events

  2. Security
    - RLS policies ensure users can only modify their own agents
    - Comprehensive audit trail for all skill changes

  3. Legion Protocol Support
    - Enables dynamic skill acquisition (Condense phase)
    - Supports skill repurposing and evolution
...
BEGIN
  ...
  WHERE logs.user_id = current_user_id
    ...
    AND (skill_name IS NULL OR logs.detail->>'skill_name' = skill_name)
